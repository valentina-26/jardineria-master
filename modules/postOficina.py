...
def guardaroficina():
    ...
    peticion = requests.post("http://172.16.103.34:5504", data=json.dumps(oficina, indent=4).encode("UTF-8"))
    res = peticion.json()
...
def DeleteOficina(id):
    ...
    if len(data):
        peticion = requests.delete(f"http://172.16.103.34:5504/oficinas/{id}")
